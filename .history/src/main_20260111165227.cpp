...
void hienThiCountdown(const char* ten, int giay, int yPos)
{
  oled.setTextSize(2);
  oled.setCursor(90,yPos);
  oled.print(giay);
  oled.display();
}
...
void doNhiet()
{
  ...
  if(conLai<0) conLai=0;
  hienThiCountdown("Nhiet",conLai,18);
...
void doNhipTim(uint32_t ir)
{
  ...
  if(conLai<0) conLai=0;
  hienThiCountdown("BPM",conLai,36);
