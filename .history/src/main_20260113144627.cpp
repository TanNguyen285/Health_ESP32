...
bool coVat = !(rawNho && (millis() - tBatDauKhongVat > TG_XAC_NHAN_KHONG_VAT));
    ...
    switch (trangThaiCan)
    {
        ...
        case CAN_DANG_DO:
        {
            ...
            if (daQua >= TG_DO_CAN_MS)
            {
                ...
                ketQuaCan = rawTB * cali;   // kg

                trangThaiCan = CAN_HIEN;
                inCountdownScreen = false;

                guiFirebase(ketQuaCan * 1000.0f); // nếu Firebase dùng g
    ...
    if (!coVat && daCoVat)
    {
        ...
        // RESET CỰC KỲ QUAN TRỌNG
        canLoc = 0;
        tongCan = 0;
        soMauCan = 0;
        tBatDauKhongVat = 0;

        Serial.println("Vat roi hoan toan, reset can");
    }
    break;
