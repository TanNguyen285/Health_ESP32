...
/* ================= HX711 FILTER + MEASURE ================= */
#define HX_LP_ALPHA        0.2f
#define HX_NGUONG_NHIEU    300
#define HX_NGUONG_CO_VAT   2000
#define HX_AUTO_TARE_MS   2000
#define HX_DO_CAN_MS      5000     // ⬅️ đo 5 GIÂY

// 1kg ≈ -12680 raw
float HX_CALI = 0.000079f;

long hxOffset = 0;
float hxLoc = 0;
unsigned long hxTKhongVat = 0;
...
void hxInit()
{
    ...
    Serial.println("HX711 OK, tare lan dau");
    hxTare();
}
void capNhatCan()
{
    ...
    /* ===== ĐỌC + LỌC HX711 ===== */
    long raw = scale.read() - hxOffset;

    hxLoc = HX_LP_ALPHA * raw + (1.0f - HX_LP_ALPHA) * hxLoc;

    if (abs(hxLoc) < HX_NGUONG_NHIEU)
        hxLoc = 0;

    hxCoVat = (hxLoc < -HX_NGUONG_CO_VAT);
    ...
    if (!hxCoVat)
    {
        ...
        if (millis() - hxTKhongVat >= HX_AUTO_TARE_MS)
        {
            hxTare();
            hxTKhongVat = 0;
            Serial.println("HX711 AUTO TARE");
        }
    }
    else
    {
    ...
    /* ===== KG (DƯƠNG) ===== */
    hxKg = hxCoVat ? (-hxLoc * HX_CALI) : 0;
    float canGram = hxKg * 1000.0f;
    ...
    switch (trangThaiCan)
    {
        case CAN_CHO_VAT:
            if (hxCoVat && !daCoVat)
            {
                ...
                soMauCan = 0;
                inCountdownScreen = true;
                Serial.println("Bat dau do can (5s)");
            }
            break;
        ...
        case CAN_DANG_DO:
        {
            ...
            int conLai = (HX_DO_CAN_MS - daQua + 999) / 1000;
            if (conLai < 0) conLai = 0;
            ...
            if (daQua >= HX_DO_CAN_MS)
            {
                ketQuaCan = tongCan / soMauCan; // gram
                trangThaiCan = CAN_HIEN;
                inCountdownScreen = false;
                guiFirebase(ketQuaCan);
                Serial.printf("Can xong: %.1f g\n", ketQuaCan);
            }
            break;
        }
