...
HX711 scale;
#define NGUONG_NHIEU    300        // raw ~ 0
#define NGUONG_CO_VAT   2000       // raw xác nhận có vật

#define TG_DO_CAN_MS    5000       // 5s đo
#define TG_KHONG_VAT_MS 3000       // 3s xác nhận rời cân

float cali = 0.000079f;            // kg / raw (đã đảo dấu)  
long hx_offset = 0;
long hx_raw = 0;
...
void capNhatCan()
{
    ...
    // =================================================
    // ===== XÁC ĐỊNH KHÔNG CÓ VẬT (RAW + 3s) =====
    // =================================================
    bool rawNho = abs(hx_raw) < NGUONG_NHIEU;
    ...
    bool khongVatOnDinh =
        (tBatDauKhongVat != 0) &&
        (millis() - tBatDauKhongVat >= TG_KHONG_VAT_MS);

    bool coVat = !khongVatOnDinh;
    ...
    switch (trangThaiCan)
    {
        // ================= CHỜ VẬT =================
        case CAN_CHO_VAT:
            if (coVat && !daCoVat && abs(hx_raw) > NGUONG_CO_VAT)
            {
        ...
        case CAN_DANG_DO:
        {
            ...
            if (daQua >= TG_DO_CAN_MS)
            {
                ...
                guiFirebase(ketQuaCan * 1000.0f);

                Serial.printf("Can xong: %.3f kg\n", ketQuaCan);
            }
            break;
        }
        ...
        case CAN_HIEN:
            if (!coVat && daCoVat)
            {
                ...
                // AUTO-TARE SAU 3s KHÔNG VẬT
                hx_offset = raw;

                Serial.println("Khong vat 3s -> reset + auto tare");
            }
            break;
    }
