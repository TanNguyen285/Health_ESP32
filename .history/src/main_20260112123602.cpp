...
void doNhipTim()
{
    ...
    if (max30102.available()) {
        ...
        // ===== BAT DINH =====
        const float PEAK_THRESHOLD = 300;      // chỉnh 200–600
        const unsigned long MIN_INTERVAL = 300; // ms (~200 BPM max)
    ...
    (now - lastBeatTime) > MIN_BEAT_INTERVAL)
{
    ...
    // BEAT ĐẦU: chỉ để ổn định, KHÔNG TÍNH
    if (beatCount == 0)
    {
        beatCount = 1;   // đánh dấu đã qua beat giả
        return;
    }
    ...
    {
        beatTimes[beatCount - 1] = now;
        Serial.printf("Beat %d | irAC=%.1f\n", beatCount, irAC);
        beatCount++;
    }
}
