...
bool sendFirebase(float weight) {
  ...
  if (httpCode > 0) {
    Serial.printf("Firebase trả về HTTP %d\n", httpCode);
...
void updateOLED(float weight) {
  ...
  display.print("H:"); display.print((long)CALIBRATION_FACTOR);
  display.print(" Ng:"); display.print((int)STABLE_THRESHOLD_G);
  display.print(" A:"); display.print((int)EMA_ALPHA_PERCENT);
...
void handleSerialCommands() {
  ...
  if (cmd == "t" || cmd == "tare") {
    ...
    Serial.println("Chú ý: để thay đổi các tham số, sửa ở khối CẤU HÌNH trong mã rồi nạp lại");
  }
}
...
void setup() {
  ...
  // Sử dụng giá trị mặc định khai báo ở khối CẤU HÌNH (nếu cần thay đổi, sửa code và nạp lại)
...
void loop() {
  ...
  if (scale.is_ready()) {
    ...
    if ((isnan(lastFirebaseWeight) || fabs(smoothedWeight - lastFirebaseWeight) >= stableThreshold) && (now - lastFirebase) >= FIREBASE_MIN_INTERVAL_MS) {
      lastFirebase = now;
