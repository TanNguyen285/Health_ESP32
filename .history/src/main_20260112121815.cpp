...
void doNhipTim()
{
    ...
    // ===== DOC DU LIEU MAX30102 =====
    if (max30102.available()) {
        float ir = (float)max30102.getIR();
        max30102.nextSample();

        // ===== LOC DC (RAT QUAN TRONG) =====
        irDC = 0.95f * irDC + 0.05f * ir;
        irAC = ir - irDC;
         Serial.print("IR raw: ");
Serial.print(ir);
Serial.print(" | IR AC: ");
    ...
    if (daQua >= TG_DO_BPM_MS) {
        ...
        if (beatCount >= 2) {
            ...
            for (int i = 1; i < beatCount; i++) {
                tong += (beatTimes[i] - beatTimes[i - 1]);
            }

            float avgInterval = (float)tong / (beatCount - 1);
            ketQuaBPM = 60000.0f / avgInterval;
        } else {
            ketQuaBPM = 0;
        }
