...
#define CHIEU_CAO_CAM_BIEN 180.0f // cm, trần nhà
#define NGUONG_PHAT_HIEN_CC 10.0f // ngưỡng phát hiện người (cm)
#define TG_DO_CC_MS 3000           // thời gian đo trung bình (ms)

// Cấu hình nút: GPIO34 (input-only trên ESP32)
#define BUTTON_PIN 34
#define BUTTON_ACTIVE_LOW 1    // 1 nếu nút kéo xuống GND khi nhấn (cần pull-up ngoài)
#define BUTTON_DEBOUNCE_MS 50

// Nếu =1 thì chỉ đo chiều cao khi nhấn nút (không tự kích hoạt bằng HC-SR04)
#define CHIEU_CAO_MANUAL_ONLY 1
...
int lastButtonReading = HIGH;     // raw reading
unsigned long lastButtonDebounceTime = 0; // ms

// Manual mode khi nhấn nút để ép đo chiều cao
bool manualChieuCaoMode = false;
...
float docKhoangCach()
{
    ...
    if (duration == 0) return -1; // trả về -1 nếu không đọc được

    float kc = duration * 0.034f / 2.0f; // cm
    // loại bỏ giá trị quá lớn hoặc <=0
    if (kc <= 0 || kc >= CHIEU_CAO_CAM_BIEN) return -1;
...
void capNhatChieuCao()
{
    ...
    bool coNguoi = (kc > 0 && kc < NGUONG_PHAT_HIEN_CC); // chỉ coi người nếu kc hợp lệ

    switch (trangThaiCC)
    {
        case CC_CHO_NGUOI:
            // Nếu đặt chế độ manual-only -> KHÔNG tự động bắt đầu khi cảm biến phát hiện người
            if (!CHIEU_CAO_MANUAL_ONLY && coNguoi && !daCoNguoiCC)
            {
        ...
        case CC_DANG_DO:
        {
            ...
            // Nếu do bằng nút thì hiển thị khác
            showCountdownScreen(manualChieuCaoMode ? "Do Chieu Cao (BTN)" : "Do Chieu Cao", conLai);

            // Chỉ cộng mẫu hợp lệ (cho phép linh hoạt khi manual)
            if (kc > 0 && kc < CHIEU_CAO_CAM_BIEN)
            {
                // Nếu ở chế độ manual, chấp nhận mẫu hợp lệ bất kể ngưỡng phát hiện người
                if (manualChieuCaoMode || kc < NGUONG_PHAT_HIEN_CC)
                {
                    tongKC += kc;
                    soMauKC++;
                }
            ...
            if (daQua >= TG_DO_CC_MS)
            {
                ...
                {
                    Serial.println("Khong co mau hop le, khong do duoc");
                    chieuCaoKetQua = 0;
                }

                trangThaiCC = CC_HIEN;
                inCountdownScreen = false;
                // Hủy chế độ manual khi kết thúc
                manualChieuCaoMode = false;
            }
            break;
        }
...
// Bắt đầu đo chiều cao bằng tay (nút nhấn)
void startChieuCaoManual()
{
    if (trangThaiCC != CC_DANG_DO && !inCountdownScreen)
    {
        trangThaiCC = CC_DANG_DO;
        tBatDauCC = millis();
        tongKC = 0;
        soMauKC = 0;
        inCountdownScreen = true;
        manualChieuCaoMode = true;
        daCoNguoiCC = true; // tránh bị coi là 'không có người' khi mới bắt đầu
        Serial.println("Manual start: Do Chieu Cao (button)");
    }
}
...
void doNhipTim(uint32_t irRaw)
{
    ...
    if (!dangDoBPM)
    {
        ...
        inCountdownScreen = true;
        Serial.println("Bat dau do BPM 30s");
    }
    ...
    // Debug: log per-sample values (only while measuring)
    Serial.printf("t=%lu daQua=%lu irRaw=%lu irDC=%.1f irAC=%.1f\n", now, daQua, irRaw, irDC, irAC);
        ...
        interval > MIN_BEAT_INTERVAL)
    {
        ...
        else if (beatCount < MAX_BEATS)
        {
            ...
            lastBeatTime = now;
            Serial.printf(">>> Beat %d | interval=%lu ms | irAC=%.1f\n", beatCount, interval, irAC);
        }
    }
    ...
    if (daQua >= TG_DO_BPM_MS)
    {
        ...
        if (beatCount >= 2)
        {
            ...
            Serial.printf("Beats=%d duration=%lu ms -> calc BPM=(%d*60000)/%lu = %d\n",
                          beatCount, duration, beatCount - 1, duration, ketQuaBPM);
        }
        else
        {
            ketQuaBPM = 0;
            Serial.printf("Beats=%d -> BPM=0 (too few beats)\n", beatCount);
        }
...
// Button (GPIO34 is input-only — ensure external pull-up/down as required)
pinMode(BUTTON_PIN, INPUT);
lastButtonReading = digitalRead(BUTTON_PIN);
buttonState = lastButtonReading;
Serial.println("Button on GPIO34 configured as INPUT; ensure external pull-up/down as required");
...
void loop()
{
  ...
  if (millis() - lastButtonDebounceTime > BUTTON_DEBOUNCE_MS) {
    if (reading != buttonState) {
      buttonState = reading;
      bool pressed = (BUTTON_ACTIVE_LOW ? (buttonState == LOW) : (buttonState == HIGH));
      if (pressed) {
        // Nếu đặt chỉ manual thì bắt đầu đo khi nhấn
        startChieuCaoManual();
      }
    }
  }
