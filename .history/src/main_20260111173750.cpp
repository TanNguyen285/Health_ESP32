...
bool phatHienTayBPM(uint32_t ir){
    return ir > IR_THRESHOLD; // vẫn dùng threshold IR để phát hiện tay
}
...
void doNhipTim(uint32_t ir)
{
    ...
    // Hiển thị countdown màn hình riêng
    ...
    if(daQua >= TG_DO_BPM_MS){
        // Tính BPM trung bình: beats trong 15s
        if(beats > 0){
            ketQuaBPM = beats * 60000 / TG_DO_BPM_MS; 
        } else {
            ketQuaBPM = 0; // Không detect được beat
        }
