...
void capNhatCan()
{
    ...
    Serial.printf("canHienTai: %.2f, canLoc: %.2f\n", canHienTai, canLoc);

    switch(trangThaiCan)
    {
        case CHO_VAT:
            // Chỉ bắt đầu đo khi vật mới đặt lên cân
            if(canLoc > NGUONG_CO_VAT)
            {
                demOnDinh++;
                if(demOnDinh >= 5) // ~400ms ổn định
                {
        ...
        case DANG_DO:
            {
                ...
                showCountdownScreen("Do Can", conLai); // OLED countdown khi đo
                ...
                if(daQua >= TG_DO_CAN_MS)
                {
                    ...
                    // Sau khi đo xong, OLED sẽ giữ kết quả
                    showMainScreen();
                    Serial.printf("Ket qua can: %.2f g\n", ketQuaCan);
                }
            }
            break;

        case HIEN_KET_QUA:
            // Giữ kết quả, không update nữa
            // Chỉ reset khi cân về 0
            if(canLoc <= NGUONG_NHIEU)
            {
                trangThaiCan = CHO_VAT;
                ketQuaCan = 0;
                Serial.println("Can ve 0, san sang lan do tiep theo");
            }
            break;
    }
}
