...
/* ================= CÂN ================= */
enum TrangThaiCan {
    CAN_CHO_VAT,     // Chưa có vật
    CAN_DANG_DO,     // Đang đo (countdown)
    CAN_HIEN         // Đã đo xong, giữ kết quả
};
...
void capNhatCan()
{
    ...
    switch(trangThaiCan)
    {
        ...
        case CAN_HIEN:
            // Giữ kết quả, reset chỉ khi vật rời cân thật sự
            if(canHienTai <= NGUONG_NHIEU)
            {
                trangThaiCan = CAN_CHO_VAT;
                ketQuaCan = 0;
                canLoc = 0;
                Serial.println("Vat roi, san sang lan do tiep theo");
            }
            break;
    }
}
